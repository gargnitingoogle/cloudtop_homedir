...
alias loadfuse1test='bucket=gargnitin-fuse-test-bucket1 && mountdir=~/work/cloud/storage/client/gcsfuse/test_buckets && mountpath=$mountdir/$bucket-mount && mkdir -pv $mountpath && logpath=$mountdir/$bucket-logfile.log && rm -rfv $logpath && (fusermount -u $mountpath || true) && cd ~/work/cloud/storage/client/gcsfuse/src/gcsfuse  && go run . --debug_fuse --debug_fuse_errors --debug_gcs --debug_http --log-file=$logpath --debug_mutex $bucket $mountpath'
alias unloadfuse1test='bucket=gargnitin-fuse-test-bucket1 && mountdir=~/work/cloud/storage/client/gcsfuse/test_buckets && mountpath=$mountdir/$bucket-mount && (fusermount -u $mountpath || true)'

alias gcsfuse='cd ~/work/cloud/storage/client/gcsfuse/src/gcsfuse'
